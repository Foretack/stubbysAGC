...
public class RotateFilter extends GenerateFilter {
    private final Vec2 v1 = new Vec2(), v2 = new Vec2(), v3 = new Vec2();

    int angle = 45;

    {
        options(new SliderOption("angle", () -> angle, f -> angle = (int) f, 0, 360, 90));
    }
    ...
    public void draw(Image image) {
        ...
        Vec2 vsize = Scaling.fit
                .apply(image.getDrawable().getMinWidth(), image.getDrawable().getMinHeight(), image.getWidth(),
                        image.getHeight());
        ...
        clamper.get(Tmp.v1.trns(angle - 90, size)
                .add(image.getWidth() / 2f + image.getX(), image.getHeight() / 2f + image.getY()));
        clamper.get(Tmp.v2.set(Tmp.v1).sub(image.getWidth() / 2f + image.getX(), image.getHeight() / 2f + image.getY())
                .rotate(180f).add(image.getWidth() / 2f + image.getX(), image.getHeight() / 2f + image.getY()));

        Lines.stroke(Scl.scl(3f), Pal.accent);
        Lines.lineAngle(Tmp.v1.x, Tmp.v1.y, Tmp.v2.x, Tmp.v2.y);
        Draw.reset();
    }
