...
public class ImpactReactor extends PowerGenerator {
    ...
    public void setBars() {
        ...
        bars.add("poweroutput", entity -> new Bar(() ->
                Core.bundle.format("bar.poweroutput",
                        Strings.fixed(
                                Math.max(entity.block.getPowerProduction(entity.tile) - consumes.getPower().usage, 0) *
                                        60 * entity.timeScale, 1)),
                () -> Pal.powerBar,
    ...
    public void draw(Tile tile) {
        ...
        for (int i = 0; i < plasmas; i++) {
            ...
            Draw.rect(reg(plasmaRegions[i]), tile.drawx(), tile.drawy(), r, r,
                    Time.time() * (12 + i * 6f) * entity.warmup);
            Draw.blend();
        }
    ...
    public void drawLight(Tile tile) {
        ...
        renderer.lights.add(tile.drawx(), tile.drawy(), (110f + Mathf.absin(5, 5f)) * fract,
                Tmp.c1.set(plasma2).lerp(plasma1, Mathf.absin(7f, 0.2f)), 0.8f * fract);
    }
